...
$(function() {
  // try some servers
  DEGAUCHE.create_connection('http://ascri.be:9292/faye');
  $('#connection_form').submit(function(e) {
    e.preventDefault();
    DEGAUCHE.change_connection($('#address').val(), DEGAUCHE.active_connection);
  })
});

DEGAUCHE.active_connection = null;


DEGAUCHE.create_connection = function(address) {
  var serv = new DEGAUCHE.Server(address);
  DEGAUCHE.servers.push(serv);
  DEGAUCHE.active_connection = serv;
  DEGAUCHE.faye = serv.new_client();
  DEGAUCHE.faye.subscribe('/messages/new', DEGAUCHE.receive_message_from_faye);
}

DEGAUCHE.change_connection = function(address, connection) {
  if(address != '') {
    connection.close();
    DEGAUCHE.create_connection(address);
  }
}
...
DEGAUCHE.Server = (function() {
  function Server(address) {
    this.address = address;
    this.connected = false;
    this.client = null;
  }
  Server.prototype.new_client = function() {
    if (this.address !== null && this.address !== '') {
      return this.client = new Faye.Client(this.address);
    } else {
      return null;
    }
  };
