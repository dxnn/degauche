DEGAUCHE.extend({
  keyword: "mistranscribe",
  name: "Mistranscriber",
  description: "Wrap text with {braces|mustaches|handlebars|curly brackets} and separate with pipes. See more usage examples at http://mistranscri.be/",
  // description: "Within degauche you should wrap with an extra pair of {{braces|mustaches|handlebars|curly brackets}}. See more usage examples at http://mistranscri.be/",
  init: function() {
    
  },
  mafipulate_received: function(packet) {
    var ext = this;
...
// from https://github.com/dxnn/mistranscribe

(function($){
  
  // the counter and extensions are stored for the whole page, not each individual invocation
  var MSTRNSCRB = {
    counter: 0,
    extensions: []
  };
  ...
  var methods = {
    ...
    parse: function(options) {
      return this.each(function() {
        ...
        while((chunks = regex.exec(html)) != null) { // THINK: !== ?
    ...
    start: function(options) {
      return this.each(function() {
        ...
        if(data.setIntervalId) {return false;}
        ...
        data.setIntervalId = setInterval(function() {
          $.each(data.transformers, function(index, transformer) {          
            var pickitem;
            for(var i = MSTRNSCRB.extensions.length - 1; i >= 0; i--) {
              if(pickitem = MSTRNSCRB.extensions[i].picker(transformer.items, transformer, data.transformers)) {i = -1;} // stop at the first match
            };

            data.transformers[index].current = pickitem;
    ...
    stop: function() {
      return this.each(function() {
        ...
        clearInterval(data.setIntervalId);
        delete(data.setIntervalId);
      });
    },
  ...
  $.fn.mistranscribe('extend', {
    ...
    picker: function(items, transformer) {
      ...
      // first pass
      // THINK: can we do this at parse time?
  ...
  $.fn.mistranscribe('extend', {
    ...
    picker: function(items, transformer, transformers) {
      var return_item;
      
      if(transformer.copy) {
        $.each(transformers, function(index, test_trans) {          
  ...
  // TODOs: degrade nicely, README, examples, base preamble extension 
